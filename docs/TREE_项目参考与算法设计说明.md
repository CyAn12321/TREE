...
**主题：** L-System 树干生成、器官实例与季节系统、分阶段风动画
**版本：** v1.1（可运行原型实现版）  
**日期：** 2026-07-20  
...
5. 动画系统分阶段实现；当前第一阶段使用 Maya bend deformer 与表达式完成树枝、细枝和附着器官的风摆动，雨雪与飘落效果暂不启用。
...
本轮已将前述“预期方案”落实到 `TREE`，并保留纯 Python 数据层与 Maya 表现层的分离。
...
| 季节固定候选池 | 同一 `attachment_id` 使用独立随机流；季节改变数量/状态时共享候选器官身份，细枝长度与叶片尺寸可使最终位置产生受控变化 | `src/foliage.py` |
| 真实器官网格 | Maya 合并网格直接由目录中的 OBJ 变换得到；原程序化叶/花保留为无资产时的回退 | `src/maya_foliage.py` |
| 第一阶段风动画 | 树干、树枝、细枝和附着器官共同使用 Maya bend deformer 与表达式摆动；纯 Python 文件保留同构参考公式 | `src/vertex_animation.py`、`src/maya_weather.py` |
...
- 当前 Maya 动画模块使用 bend deformer + expression 表达树冠风摆动，并使用与树上 foliage 同源的关键帧实例实现落叶和落花，不依赖 Maya Dynamics Solver。
- 风动画组使用 `animationType=wind_sway`、`falling_organs` 或组合值，并使用 `implementationVersion=weather-keyed-fall-1.9` 标记；落叶和落花从实际叶片/花瓣位置生成，并沿慢速、带风向偏移的多段曲线下落；刷新时会先删除旧的风/天气管理组和遗留动态节点。
- 雨、雪和积雪仍属于后续独立阶段，尚未重新实现。
