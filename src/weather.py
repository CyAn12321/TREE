...
def build_weather_plan(tree_model, foliage_model=None, config=None):
    ...
    values = {
        ...
        "wind_curvature": 0.40 * config.wind_intensity,
        "wind_frequency": 0.045 + 0.085 * config.wind_intensity,
        "rain_rate": int(round(1600.0 * config.rain_intensity)),
        "rain_pool_count": 2000,
        "rain_speed": 14.0 + 20.0 * config.rain_intensity,
        "rain_plane_height": maximum[1] + max(size[1], 3.0),
        "rain_plane_width": max(size[0], size[2], 4.0) * 1.6,
        "rain_plane_depth": max(size[0], size[2], 4.0) * 1.6,
